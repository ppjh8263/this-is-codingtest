...
def main(N,numbers):
    ...
    for i in range(1,len(numbers)+1,1):
        combination_result += list(map(sum,combinations(numbers,i)))

    return min(set([i for i in range(1,max(combination_result)+2,1)])- set(combination_result))

@check_time
def answer_book(N,numbers):
    # numbers
